...
window.CodeMirror = (function() {
  ...
  var gecko = /gecko\/\d/i.test(navigator.userAgent),
      ...
      // Adapted from http://detectmobilebrowsers.com/download/javascript.
  ...
  flipCtrlCmd = mac && (qtwebkit || opera && (opera_version == null || opera_version < 12.11));
  ...
  function CodeMirror(place, options) {
    ...
    themeChanged(this, options.addThemeToBodyClass);
  ...
  function themeChanged(cm, changeBodyClass) {
	// ADDITION: New option, addThemeToBodyClass, allows the body's class to optionally contain the
	//			 theme name as well (for skinning parts of the page with CodeMirror's theme changing
	//			 system).
	if (typeof(changeBodyClass) === 'undefined') changeBodyClass = cm.options.addThemeToBodyClass;
    if (changeBodyClass)
	  document.body.className = (document.body.className.replace(/\s*cm-s-.*\S+/g, "") + cm.options.theme.replace(/(^|\s)\s*/g, " cm-s-")).trim();
  ...
  option("theme", "default", function(cm) {
    themeChanged(cm, cm.options.addThemeToBodyClass);
    guttersChanged(cm);
  }, true);
  option("addThemeToBodyClass", true);
